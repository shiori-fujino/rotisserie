...
# -*- coding: utf-8 -*-
"""
22 Rydalmere — DB-enabled scraper (WordPress JSON roster page)
Structure cloned from 45 Granville scraper.
"""
...
# ---------- constants ----------
BASE_URL = "https://22rydalmere.com.au"
API_URL  = f"{BASE_URL}/wp-json/wp/v2/pages/26"
...
def parse_girls(html: str):
    ...
    girls, paragraphs = [], soup.find_all("p")

    for i, p in enumerate(paragraphs):
        txt = p.get_text(" ", strip=True)
        # detect new entry by tilde and country prefix
...
def scrape():
    ...
    with open("22rydalmere.json", "w", encoding="utf-8") as f:
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ensure_schema(cur)
                shop_id = get_or_create_shop(cur, "22 Rydalmere", BASE_URL)
                inserted = 0
                for entry in out:
